...
namespace WebSite.mobile.weixinpay
{
     ...
     * 
     * */
    public partial class weipay : m_basepage
    {
        //页面输出 不用操作
        ...
        private void BindData()
        {
            ...
            this.lblPrice.Text = "￥" + Common.Utils.decimalObjectToStr(decimal.Parse(this.TotalFee) / 100);
            ...
            {
                Response.Write("<script>alert('订单编号为空');history.go(-1);</script>");
                Response.End();
            }
            member_join_orderInfo info = BLL.member_join_orderBLL.GetModel(OrderSN);
            if (info == null || !info.OrderId.Trim().Equals(OrderSN))
            {
                Response.Write("<script>alert('订单编号不存在');history.go(-1);</script>");
                Response.End();
            }
            decimal _TotalFee = decimal.Parse(TotalFee);
            if ((info.Amount * 100) != _TotalFee)
            {
                Response.Write("<script>alert('订单金额不对');history.go(-1);</script>");
                Response.End();
            }
        }
        ...
        //{
        //    orderInfo orderinfo = BLL.orderBLL.GetModel(orderid);
        ...
        //    this.OrderSN = orderinfo.orderno;
        //    this.Body = companyname + "-生活超市购物";
        //    this.TotalFee = ((int)(orderinfo.totalamount * 100)).ToString();
        //    this.Attach = uid.ToString();
        //    this.UserOpenId = Common.CommFun.RequestOpenid();// this.Request.QueryString["UserOpenId"];
