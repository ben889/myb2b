<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="index.aspx.cs" Inherits="WebSite.mobile.index" %>
...
<head>
    ...
    <meta name="Keywords" content="<%=base.site_title %>" />
    <meta name="Description" content="<%=base.site_title %>" />
    <title><%=base.site_title %></title>
    <link rel="stylesheet" href="css/jquery.mobile.css" type="text/css" media="screen" />
    ...
    <script>
        $(function () {
            bindmessgaelist();
            window.setInterval(bindmessgaelist, 12000);
        });
        function bindmessgaelist() {
            //alert("执行绑定消息");
            $("#list").html("");
            var ajaxdata = { ajaxmethod: "bindmessgaelist" };
            $.ajax({
                type: "POST",
                url: "index.aspx",
                data: ajaxdata,
                ...
                success: function (result) {
                    ...
                    if (unread > 0)
                        $("#sms_count").html("<a class=\"tips\" style=\"top:-4px;\">" + unread + "</a>");
                    else
                        $("#sms_count").html("");
                    addrows(result.rows);
                },
                error: function () {
                    //alert("价格加载失败，请刷新页面重试");
                }
            });
        }
        function addrows(json) {
            if (json.length > 0) {
                for (var i = 0; i < json.length; i++) {
                    ...
                    if (status == "1")
                        status = "<span style='color:green;'>[已读]</span>";
                    else
                        status = "<span style='color:red;'>[未读]</span>";
                    var rowhtml = "<a href=\"member/viewmessage.aspx?id=" + json[i].id + "\">" + status + " " + json[i].title + "<span class=\"jt\"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
                    $("#list").append(rowhtml);
                }
            }
            else {
...
<body>
    <div id="header">
        <ul>
            <li class="img"></li>
            <li class="search"><a style="display: block; width: 44px; height: 44px;"></a></li>
        </ul>
        <span class="qy"></span><span class="tit"><a href="/mobile/index.aspx" data-ajax="false" class="ui-link">&nbsp;<%=base.site_title %>&nbsp;</a></span>
    </div>
