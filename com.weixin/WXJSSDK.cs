...
public class WXJSSDK
{
    ...
    private string getJsApiTicket()
    {
        ...
        int expire_time = ConvertDateTimeInt(DateTime.Now);
        string ticket = "";
        string accessToken = Context.GetAccessToken(appId, appSecret);//获取系统的全局token 
        if (expire_time < ConvertDateTimeInt(DateTime.Now) || ticket.Trim().Length == 0)
        {
